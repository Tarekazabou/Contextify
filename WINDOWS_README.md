...
4. **Use it!**
   ```powershell
   .\contextify.bat 'add a dark mode toggle'
   ```
...
.\contextify.bat 'add a dark mode toggle'
.\contextify.bat 'create a user profile card' --focus frontend
.\contextify.bat 'fix the login bug' --changed
.\contextify.bat 'add API endpoint' --focus backend
```
...
# Add this line:
function ctx { & "C:\path\to\contextify\contextify.bat" $args }
...
# Add to your $PROFILE:
function ctx { & "C:\path\to\contextify\contextify.bat" $args }
function ctxf { & "C:\path\to\contextify\contextify.bat" $args --focus frontend }
function ctxb { & "C:\path\to\contextify\contextify.bat" $args --focus backend }
function ctxc { & "C:\path\to\contextify\contextify.bat" $args --changed }
...
```powershell
$env:GEMINI_API_KEY='your-key'; .\contextify.bat 'your request'
```

### Save Output to File

```powershell
.\contextify.bat 'major refactor' --output prompt.md
```
...
```powershell
.\contextify.bat 'request' --output prompt.txt
```
...
# Generate prompt
.\contextify.bat 'add dark mode to navbar' --focus frontend

# The prompt is now in your clipboard!
# Paste into GitHub Copilot, ChatGPT, or Cursor

# Or save to file
.\contextify.bat 'add dark mode' --output dark-mode-prompt.md
```
...
```powershell
.\contextify.bat 'create UserCard component' --focus frontend
.\contextify.bat 'add form validation' --focus frontend
.\contextify.bat 'style the dashboard' --focus frontend
```

### Backend Development
```powershell
.\contextify.bat 'add POST /api/users endpoint' --focus backend
.\contextify.bat 'add authentication middleware' --focus backend
.\contextify.bat 'implement rate limiting' --focus backend
```

### Bug Fixes
```powershell
# After making changes
.\contextify.bat 'fix the TypeError in form submission' --changed
```

### Database Work
```powershell
.\contextify.bat 'add posts table migration' --focus database
.\contextify.bat 'add user relationships' --focus database
```
  ...
  "tasks": [
    {
      "label": "Contextify",
      "type": "shell",
      "command": "C:\\path\\to\\contextify\\contextify.bat",
      "args": ["${input:request}"],
      "problemMatcher": []
    }
  ],
...
3. ✅ Try: `.\contextify.bat 'your first request'`
